...
mygmod <- gmod() + 
  ...
  rootlet(pRecover=choose(c(r_S1H, 0), 
                          when=c(state=="S1", Inf)),  
          transformation =rate2prob) + 
  rootlet(pGetSick=if(state=="H"){r_HS1}else{0}, transformation=rate2prob) + 
  rootlet(name=pProgress,
          formula=choose(r_S1S2 * state=="S1"))
  rootlet(pProgress=if(state=="S1"){r_S1S2}else{0}*
            if(decision%in%c("StrategyB","StrategyAB")){hr_S1S2_trtB}else{1}, 
          pProgress=aggregate(choose(c(r_S1S2, 0), 
                                     when=c(state=="S1", Inf)), 
                              choose(c(hr_S1S2_trtB, 1), 
                                     when=c(decision%in%c("StrategyB","StrategyAB"), Inf)), 
                              fun = prod), 
          transformation=rate2prob) + 
  rootlet(pDie=if(state=="D"){0}else{r_HD}*
            if(state=="S1"){hr_S1}
          else if(state=="S2"){hr_S2}
          else {1},
          name = pDie,
          formula=aggregate(r_HD := state!="D",
                            hr_S1 := state=="S1",
                            hr_S2 := state=="S2", 
                            fun = prod), 
          transformation=rate2prob) + 
  ...
  rootlet(name = cost, 
          formula=SUM_IF(c_trtA, state %in% c("S1","S2") & decision %in% c("StrategyA","StrategyAB"),
                         ...
                         c_D, state=="D"), 
          name = cost, 
          formula=aggregate(c_trtA * (state %in% c("S1","S2") & decision %in% c("StrategyA","StrategyAB")),
                         c_trtB * (state %in% c("S1","S2") & decision %in% c("StrategyB","StrategyAB")),
                         c_H * state=="H",
                         c_S1 * state=="S1",
                         c_S2 * state=="S2",
...
pProgress <- function(state, decision){
  r_S1S2 * (state=="S1") * # progress only in S1
    hr_S1S2_trtB ^ (decision %in% c("StrategyB", "StrategyAB")) %>% # hazard ratio if deicsion has B in it
    rate2prob(rProgress)
}
```
...
```{r}
pDie <- function(state){
  r_HD*(state != "D") * # background rate of  death if not already dead
    hr_S1 ^ (state == "S1") * # HR S1 if state == S1
    hr_S2 ^ (state == "S2") %>% # HR S2 if state == S2
    rate2prob()
