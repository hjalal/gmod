...
```{r}
my_markov_model(params)
```

## Simplified example with less equations
Then, we build the gmod Markov model by using the `gmod_build()` function.
```{r}
...
To evaluate the model, we run `gmod_evaluate()` which returns the numerical results. 
```{r}
model_res2 <- gmod_gen_model_function(model_struc2,  model_function_name = "my_markov_model2")
#model_res
```

```{r}
my_markov_model2(params)
```
