---
title: "Time Dependent Markov Model"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Time Dependent Markov Model}
...
This vignette replicates our Advanced tutorial for Markov models in *Medical Decision Making*:  
...
mygmod <- gmod(model_type = "Markov", n_cycles = 75) + 
  ...
  event_mapping(event = "get_event",  
                values = c("recover", "getsick", "progress", "die", "stay"), 
                results = c("H", "S1", "S2", "D", curr_state()), 
                probs = c(pRecover(state), 
                          pGetSick(state), 
                          pProgress(state, decision, cycle_in_S1), 
                          pDie(state, cycle, v_r_mort_by_age), 
                          Inf)) +  
  payoffs(cost = cost(state, decision, get_event), 
...
```{r}
cost <- function(state, decision, get_event){
  ...
  trans_cost_dying <- (get_event=="die")*ic_D # increase in cost when dying
