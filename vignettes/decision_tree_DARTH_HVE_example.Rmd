...
We will first define the gmod model and the parameters and then we will show how one would code the model direclty in R and compare the results. 
...
mygmod <- gmod(model_type = "Decision") + 
  ...
  event_mapping(event = "DIE",  
            values = c(T, F), 
            results = c("Death", "HVE_event"), 
            probs = c(pDie(decision), Inf)) + 
  event_mapping(event = "HVE_event",  
            values = c(T, F), 
...
```{r}
gmod_evaluate(model_struc = model_struc, 
               model_function_name = "my_dec_model",
               print_model_function = TRUE)
...
# Probabilities
p_HVE          = p_HVE, 
p_HVE_comp     = p_HVE_comp, 
p_OVE_comp     = p_OVE_comp, 
p_HVE_comp_tx  = p_HVE_comp_tx, 
p_OVE_comp_tx  = p_OVE_comp_tx, 
p_biopsy_death = p_biopsy_death, 
# Costs
c_VE           = c_VE, 
c_VE_comp      = c_VE_comp,
c_tx           = c_tx, 
...
q_VE           = q_VE, 
q_VE_comp      = q_VE_comp, 
q_loss_biopsy  = q_loss_biopsy, 
q_death_biopsy = q_death_biopsy
)

# call your generated model, by simply: 
my_dec_model(params)
```
