---
title: "Advanced Markov Tutorial for Time Dependent Markov Model"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Advanced Markov Tutorial for Time Dependent Markov Model}
...
The results may be slightly different from the tutorial. The reason is that the tutorial applies a within cycle correction and also assumes that transition rewards happen after the transition.  gmod links the transition rewards to the events so they occur during each cycle. 
...
```{r}
mygmod <- gmod(model_type = "Markov", n_cycles = 25) + 
...
Since the tutorial assumes that all the events happen simultaneously (i.e., no sequence), we add a single `event_mapping()` layer we model the individual outcomes separately.  We can interpret the event_mapping as the following: The event `get_event` can have 5 values: recover, getsick, progress, die, or stay in the same state.  These values will result in the following states: H, S1, S2, D and staying in the current state, respectively. The prob argument defines the probability of these outcomes.  For example, pRecover() defins the probability of recovering and it is a function of state because this probability is only applied if the state is "S1", and is 0 otherwise. Note here we use `Inf` here is as a placeholder to reduce the amount of typing required. Then, we define the payoffs of cost and utility and define each as a function of the state and the decision taken.

The `cycle_in_S1` is a special keyward that consists of the prefix `cycle_in_` followed by one of the state names.  This keyword tells gmod which state to expand by adding additional tunnel states. You can create other tunnels by adding `cycle_in_[State]`.  Note, adding tunnel states expands the state space and can be computationally expensive. 
...
pProgress <- function(state, decision, cycle_in_S1){
  ...
  r_S1S2_tunnels <- (cycle_in_S1*r_S1S2_scale)^r_S1S2_shape - 
                    ((cycle_in_S1 - 1)*r_S1S2_scale)^r_S1S2_shape
...
utility <- function(state, decision, get_event){
  ...
  u_state <- switch(state, 
         ...
         "S2" = u_S2,
         "D" = u_D)
  return(du_HS1 + u_state + trans_util_getting_sick)
}
```
...
To evaluate the model, we run `gmod_evaluate()` which returns the numerical results. 
```{r}
...
Then, we build the gmod Markov model by using the `gmod_build()` function.
```{r}
model_struc2 <- gmod_build(mygmod, params = params, simplify = TRUE)
model_struc2
```
The output is the structure of the model including the formulae.  
...
To evaluate the model, we run `gmod_evaluate()` which returns the numerical results. 
```{r}
gmod_gen_model_function(model_struc2, model_function_name = "my_markov_model2")
#model_res
```
...
my_markov_model2(params = params)
```
...
We can load individual results here that we are interested in. For example, if we are interested in the Markov Trace of the Standard of Care treatment, we can type: 
```{r}
model_res$Trace$StandardOfCare
```

Similarly, we can load the summary results of the costs and utility of the strategies, such that:
```{r}
model_res$Summary
```
