...
pRecover <- function(state){
  ...
  # rate2prob(rRecover)
  # ifelse(state=="S1", r_S1H, 0) %>% 
  #   rate2prob()
  r_S1H*(state=="S1") %>% rate2prob()
}
```
...
pGetSick <- function(state){
  # ifelse(state=="H", r_HS1, 0) %>% 
  #   rate2prob()
  r_HS1*(state=="H") %>% rate2prob()
...
```{r}
# Function for change in cumulative hazard of a weibull (See advanced Markov tutorial)
delta_cum_hazard_weibull <- function(cycle_in_state){
  (cycle_in_state*r_S1S2_scale)^r_S1S2_shape - ((cycle_in_state - 1)*r_S1S2_scale)^r_S1S2_shape
}
...
pProgress <- function(state, decision, cycle_in_state){
  ...
  #   0
  # }
  delta_cum_hazard_weibull(cycle_in_state)^(state=="S1") * 
    hr_S1S2_trtB^(decision %in% c("StrategyB", "StrategyAB")) %>%
    rate2prob()
  
  # OR
  # ifelse(state=="S1",1,0)*
  #   ifelse(decision %in% c("StrategyB", "StrategyAB"), hr_S1S2_trtB, 1)*
  #   delta_cum_hazard_weibull(cycle_in_state) %>%
  #   rate2prob()
}
...
```{r}
state <- "S1"
# these three statements are equivalent and both return TRUE:
(state=="S1")
if (state=="S1") TRUE else FALSE
ifelse(state=="S1",TRUE,FALSE)
...
pDie <- function(state, cycle){
  ...
  # OR:
  # ifelse(state=="D", 0, v_r_mort_by_age[cycle]) * # if already dead 0 otherwise base mortality
  #   ifelse(state=="S1", hr_S1, 1) * # S1 has an relative risk of 3
  #   ifelse(state=="S2", hr_S2, 1) %>% # S2 has an relative risk of 10
  #   rate2prob()
  
  # OR:
  v_r_mort_by_age[cycle]*(state!="D") * # background mortality if not "D" multiplied by 
    hr_S1^(state=="S1") * # hazard ratio of S1 if S1 *, or 
    hr_S2^(state=="S2") %>% # hazard ratio of S2 if S2
    rate2prob()
...
cost <- function(state, decision, get_event, die){
  ...
  # c_decision <- 
  #   ifelse(state %in% c("S1","S2") & decision %in% c("StrategyA","StrategyAB"), c_trtA, 0) + 
  #   ifelse(state %in% c("S1","S2") & decision %in% c("StrategyB","StrategyAB"), c_trtB, 0)
  # 
  # 
  #   
  ...
  #        "D" = c_D)
  # 
  # trans_cost_getting_sick <- ifelse(get_event=="getsick",ic_HS1,0) # increase in cost when transitioning from Healthy to Sick
  # trans_cost_dying <- ifelse(die==TRUE,ic_D,0) # increase in cost when dying
  # 
  # # combine all
  # return(c_decision + c_state + trans_cost_getting_sick + trans_cost_dying)
  
#OR: 
  c_trtA * (state %in% c("S1","S2") & decision %in% c("StrategyA","StrategyAB")) + 
    c_trtB * (state %in% c("S1","S2") & decision %in% c("StrategyB","StrategyAB")) + 
    c_H * (state == "H") + 
    c_S1 * (state == "S1") + 
    c_S2 * (state == "S2") + 
    c_D * (state == "D") + 
    ic_HS1 * (get_event=="getsick") + 
    ic_D * (die==TRUE)
...
utility <- function(state, decision, get_event){
  # disutility of getsick event
  # trans_util_getting_sick <- ifelse(get_event=="getsick", -du_HS1, 0)
  # u_state <- switch(state, 
  #        "H" = u_H,
  #        "S1" = ifelse(decision %in% c("StrategyA", "StrategyAB"), u_trtA, u_S1),
  #        "S2" = u_S2,
  #        "D" = u_D)
  ...
  # OR: 
  u_H * (state == "H") + 
    u_S1*(state=="S1" & decision %not_in% c("StrategyA", "StrategyAB")) + 
    u_trtA*(state=="S1" & decision %in% c("StrategyA", "StrategyAB")) + 
    u_S2 * (state=="uS2") + 
    u_D * (state == "D") + 
    -du_HS1 * (get_event=="getsick")
}
```
...
model_results <- my_markov_model(model_struc, params, return_trace = T, return_transition_prob=T)
...
```{r}
gmod_expand_functions(mygmod, excel_file_name = "expanded_functions.xlsx")
df_pDie
