...
mygmod <- gmod() + # for illustration it is 75 in the tutorial 
  ...
  event(name = "get_event",  
        results = c("recover", "getsick", "progress", "stay"), 
...
```{r}
# mygmod <- gmod(model_type = "Markov", n_cycles = 5) + # for illustration it is 75 in the tutorial 
#   decisions("StandardOfCare", "StrategyA", "StrategyB", "StrategyAB") + 
#   states("H", "S1", "S2", "D") + 
#   tunnels(states = c("S1", "S2"), lengths = c(NA, NA)) + 
...
```{r}
params <- list(
  n_cycles = 10, # number of cycles is required for markov models
### Transition rates (annual), and hazard ratios (HRs) ----
r_HS1  = 0.15,  # constant annual rate of becoming Sick when Healthy
r_S1H  = 0.5 ,  # constant annual rate of becoming Healthy when Sick
hr_S1  = 3   ,  # hazard ratio of death in Sick vs Healthy 
...
```{r}
n_cycles <- 10
model_struc <- gmod_gen_model_function(mygmod, n_cycles)
#model_res
```
