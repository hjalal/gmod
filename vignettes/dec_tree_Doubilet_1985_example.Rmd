...
* How to make the probability of an event outcome based on the value of a prior (upstream) event
...
This structure is meant to reproduce the tree structure as presented in Doubilet et al.  However, their tree uses a binary biforcation, where each event can either be TRUE or FASLE.  Below we simplify this tree structure by adding more outcomes to each decision branch.  The results are identical. 

## Binary event outcomes similar to Doubilet 1985
```{r}
...
knitr::include_graphics("Doubilet_1985_diagram_OpenTree.png")

```


## Multiple outcomes per event
The impact of reducing the binary outcomes on the gmod structure.
```{r}
